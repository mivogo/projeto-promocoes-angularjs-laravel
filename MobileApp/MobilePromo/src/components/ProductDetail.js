...
import { AsyncStorage, View, Text, Image, Linking, Dimensions } from 'react-native';
...
class ProductDetail extends Component {
  ...
  addRemoveFavorite(){
    ...
    if(this.state.isFavorite){
      const url = 'http://vps415122.ovh.net/api/profile/favorites/delete/' + this.props.product.product_id;
      ...
      const url = 'http://vps415122.ovh.net/api/profile/favorites/' + this.props.product.product_id;
      const auth = 'bearer ' + this.state.token;
      console.log('FAVORITE AUTH');
      console.log(auth);
      fetch(url, {
  ...
  checkIsFavorite(nextProps){
    ...
    console.log(this.props.favorites);
      for (let i = 0; i < nextProps.favorites.length; i++) { 
        console.log(this.props.product.product_id + ' - ' + nextProps.favorites[i].product_id);
  ...
  renderButtonCart(){
    ...
    const multPrice = (this.props.product.price * this.state.quantity).toFixed(2).toString().replace('.', ',');
    if(this.state.quantity == 0) {
      return(<Button
        ...
        onPress={() => {
          AsyncStorage.getItem('@Cart').then((cart) => {
          cart = JSON.parse(cart);
          products = JSON.parse(('{ "quantity": ' + 1 + ', "product_id": "' + this.props.product.product_id + '", "data": ' + JSON.stringify(this.props.product) + '}'));
      ...
      <Button
        ...
        <Button
        style={{borderColor: 'green', flex:1 }}
        >
        <View style={{ flex:1, flexDirection:'column', alignItems: 'center' }}>
         <Text>{this.state.quantity} uni</Text> 
         <Text>{multPrice}€</Text>
         </View>
        </Button>
  ...
  render() {
    const { name, brand, category, image, link, price, weight, weight_type, price_weight, type_weight, subcategory } = this.props.product;
    const {
      thumbnailStyle,
      headerContentStyle,
